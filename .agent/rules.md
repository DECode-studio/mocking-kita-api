...
2. **FE tidak boleh import BE server module**
   - `src/client/presentation/**`, `src/client/domain/**`, `src/client/data/**`, dan `src/core/di/**` pada flow FE tidak boleh mengimpor `src/server/**`, route handler, Prisma, filesystem, cookies, atau server-only core.

3. **BE tidak boleh import FE layer**
   - `src/app/api/**` dan `src/server/**` tidak boleh mengimpor `src/client/presentation/**`, FE hook/view model, atau FE remote repository.
...
- FE data/API client tidak boleh bicara langsung ke Prisma, DB helper, filesystem, atau BE server module.
...
src/
  ...
  server/                       # BE server modules, NestJS-like ownership
    <feature>/
      index.ts
  ...
  di/
    container.ts (getService)          # FE dependency wiring
```

Catatan migrasi:
- Existing `src/client/domain/**`, `src/client/data/**`, dan `src/core/di/**` tetap menjadi flow FE.
- Existing route di `src/app/api/**` tetap hidup sebagai adapter.
- BE server module baru ditambahkan incremental di `src/server/**`.
...
### A. `src/client/presentation/**`

**Boleh**
- Import FE use case dari `src/core/di/container.ts (getService)`.
...
**Dilarang**
- Import React, Next.js, Prisma, BE server module, HTTP client, filesystem, cookies.
...
- Import BE server module public API dari `src/server/<feature>/index.ts`.
...
**Dilarang**
- Import `src/client/presentation/**`, FE remote repository, atau FE hook.
...
| :--- | :--- | :--- |
| `presentation/` | `domain/`, `di/`, shared UI, browser-safe core | `server/`, `app/api/`, Prisma, `core/db`, `core/server` |
| `domain/` | pure type/helper shared | `presentation/`, `data/`, `server/`, `next`, `react`, HTTP client |
| `data/` FE | `domain/`, `core/http-client`, `core/client`, shared core | `server/`, `app/api/`, Prisma, `core/db`, `core/server` |
| `app/api/` | `server/`, `core/server`, shared core | `presentation/`, FE hooks, FE repositories |
| `server/` BE | `core/server`, shared core, public module APIs | `presentation/`, FE `data/`, route handlers internal |
...
2. Jangan import `src/server/**` dari FE.
3. Jangan import `src/client/data/**` FE remote repository dari BE service.
