...
export function useRequestScenarioEditor({
  ...
  const activeUseCase = useMemo(() => useCase || getService(CLIENT_DI_TOKENS.apiDetailUseCase), [useCase]);
  ...
  const loadData = useCallback(async () => {
    try {
      setIsLoading(true);
      const data = await activeUseCase.load(projectId, apiId, scenarioId);
      setDetail(data);
    } catch (error) {
      addToast({
        type: 'error',
  ...
  }, [apiId, projectId, scenarioId, activeUseCase, addToast]);
  ...
  const handleSubmit = async (e?: FormEvent) => {
    ...
    try {
      setIsSaving(true);
      if (scenarioId) {
        await activeUseCase.updateRequestScenario(scenarioId, {
          name: trimmedName,
          description: '',
          matchType,
          matchStrategy,
        ...
        await activeUseCase.createRequestScenario({
          apiId,
          name: trimmedName,
          description: '',
          matchType,
