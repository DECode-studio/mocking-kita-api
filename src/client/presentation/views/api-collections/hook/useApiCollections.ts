...
import { ApiUseCase } from '@/src/client/domain/api/usecase/api_usecase';
import { Collection } from '@/src/client/domain/collection/entity/collection';
import { CollectionUseCase } from '@/src/client/domain/collection/usecase/collection_usecase';
...
export function useApiCollections(
  customApiUseCase?: ApiUseCase,
  customCollectionUseCase?: CollectionUseCase,
  embeddedProjectId?: string,
  initialApis: ApiCollection[] = [],
  initialCollections: Collection[] = []
) {
  const apiUseCase = customApiUseCase || getService(CLIENT_DI_TOKENS.apiUseCase);
  const collectionUseCase = customCollectionUseCase || getService(CLIENT_DI_TOKENS.collectionUseCase);
  ...
  const groupedApisMap = useMemo(() => {
    ...
    filteredApis.forEach((api) => {
      if (api.collectionId) {
        const group = map.get(api.collectionId) || [];
        group.push(api);
        map.set(api.collectionId, group);
      }
    });
    return map;
  }, [filteredApis]);

  const ungroupedApis = useMemo(() => {
    return filteredApis.filter((api) => !api.collectionId);
  }, [filteredApis]);
