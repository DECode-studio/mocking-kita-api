...
import { ApiDetailSnapshot, ApiDetailUseCase } from '@/src/client/domain/api/usecase/api_detail_usecase';
...
export function useApiDetail(
  customApiDetailUseCase?: ApiDetailUseCase,
  initialDetail: ApiDetailSnapshot = emptyDetailState
) {
  const apiDetailUseCase = customApiDetailUseCase || getService(CLIENT_DI_TOKENS.apiDetailUseCase);
  ...
  useEffect(() => {
    if (detail.requestScenarios.length > 0 && progress.createRequestScenario === 'not-started') {
      completeStep('createRequestScenario');
    }
  }, [detail.requestScenarios.length, progress.createRequestScenario, completeStep]);

  useEffect(() => {
    if (detail.activeResponseScenarios.length > 0 && progress.createResponseScenario === 'not-started') {
      completeStep('createResponseScenario');
    }
  }, [detail.activeResponseScenarios.length, progress.createResponseScenario, completeStep]);
