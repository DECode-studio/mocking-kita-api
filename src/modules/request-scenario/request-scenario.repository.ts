...
export async function createRequestScenario(
  ...
  const row = await prisma.requestScenario.create({
    data: {
      ...
      bodyRules: (input.bodyRules as Prisma.InputJsonValue) ?? Prisma.DbNull,
      strictBodyStructure: input.strictBodyStructure !== undefined ? input.strictBodyStructure : true,
...
export async function updateRequestScenario(id: string, input: Partial<RequestScenario>): Promise<RequestScenario> {
  ...
  const updateData: Prisma.RequestScenarioUncheckedUpdateInput = {
    ...
    ...(input.bodyRules !== undefined && { bodyRules: (input.bodyRules as Prisma.InputJsonValue) ?? Prisma.DbNull }),
    ...(input.strictBodyStructure !== undefined && { strictBodyStructure: input.strictBodyStructure }),
