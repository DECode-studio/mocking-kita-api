  ...
  return (
    <div className="space-y-1.5">
      ...
      {mode === 'raw' ? (
        ...
        <div className="space-y-2 border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 bg-slate-50/50 dark:bg-slate-950/20 max-h-56 overflow-y-auto">
          {rows.map((row, index) => {
            const isValDisabled = row.operator === 'null' || row.operator === 'empty_array';
            return (
              <div key={index} className="flex items-center gap-1.5">
                <StatusSwitch
                  checked={row.enabled}
                  onCheckedChange={() => updateRow(index, { enabled: !row.enabled })}
                  size="sm"
                />
                <input
                  ...
                  placeholder="Key"
                  className={`w-1/4 min-w-[80px] px-2 py-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md text-slate-900 dark:text-slate-100 focus:outline-none text-xs ${
                    !row.enabled ? 'line-through opacity-50' : ''
                  }`}
                />
                <select
                  id={idPrefix ? `${idPrefix}-row-${index}-operator` : undefined}
                  value={row.operator}
                  onChange={(e) => updateRow(index, { operator: e.target.value as ParamMatchOperator })}
                ...
                <input
                  ...
                  type="text"
                  value={isValDisabled ? `(${row.operator})` : row.value}
                  ...
                  className={`flex-1 px-2 py-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md text-slate-900 dark:text-slate-100 focus:outline-none text-xs ${
