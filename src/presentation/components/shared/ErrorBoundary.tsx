...
export class ErrorBoundary extends Component<Props, State> {
  ...
  public render() {
    if (this.state.hasError) {
      const isMetaMask = this.state.error?.message?.toLowerCase().includes('metamask');
      if (isMetaMask) {
      ...
      return (
        <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6">
          <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl text-center space-y-4">
            ...
            <p className="text-xs text-slate-400 leading-relaxed font-mono bg-slate-950 p-3 rounded border border-slate-800 text-left overflow-auto max-h-32">
