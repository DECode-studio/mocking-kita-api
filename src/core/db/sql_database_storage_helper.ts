...
export async function generateDatabaseSqlDump(mode: 'upsert' | 'replace' = 'upsert'): Promise<string> {
  ...
  if (environments.length > 0) {
    ...
    for (const e of environments) {
      chunks.push(
        `INSERT INTO "tblEnvironment" ("id", "project_id", "name", "environment_type", "public_base_url", "origin_base_url", "status", "created_at", "updated_at", "deleted_at") VALUES (${escapeSqlString(
          e.id
        )}, ${escapeSqlString(e.projectId)}, ${escapeSqlString(e.name)}, ${escapeSqlString(
          e.environmentType
        )}, ${escapeSqlString(e.publicBaseUrl)}, ${escapeSqlString(e.originBaseUrl)}, ${escapeSqlBoolean(
          e.status
        )}, ${escapeSqlDate(e.createdAt)}, ${escapeSqlDate(e.updatedAt)}, ${escapeSqlDate(
          e.deletedAt
        )}) ON CONFLICT ("id") DO UPDATE SET "name" = EXCLUDED."name", "environment_type" = EXCLUDED."environment_type", "public_base_url" = EXCLUDED."public_base_url", "origin_base_url" = EXCLUDED."origin_base_url", "status" = EXCLUDED."status", "updated_at" = EXCLUDED."updated_at", "deleted_at" = EXCLUDED."deleted_at";`
      );
    }
    chunks.push(``);
  }
